...
const formSchema = z.object({
  email: z.string().email().toLowerCase().trim().transform(email => email),
  // 5. toLowerCase, .trim() 같은 메서드를 사용한 값 변환이 가능하다.
  // 6. .transform() 메서드로 값을 커스텀하게 변환 가능하다.
  name: nameSchema,
  password: z.string().regex(/[a-z0-9]/g, '소문자만 가능'),
  confirm_password: z.string()
  // }).refine(checkPasswords, 'Both password should be the same!');
  // 3. 두 값을 비교할 때는 zod object에 refine 사용 가능하다.
  // 다만 이 값은 fieldError가 아닌 form에러에 나타난다.
  // 이때 에러 객체에 타겟을 정해주면 field에 보내줄 수 있다.
  // 4. regex메서드를 이용해 정규표현식을 이용한 검증이 가능하다.
